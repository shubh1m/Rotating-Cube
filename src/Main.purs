...
drawCube ctx vert3D = strokePath ctx do
    for_ faces $ \face -> do
        ...
        vd1 <- fromMaybe (Point3D 0.0 0.0 0.0) v1
        vd2 <- fromMaybe (Point3D 0.0 0.0 0.0) v2
        vd3 <- fromMaybe (Point3D 0.0 0.0 0.0) v3
        vd4 <- fromMaybe (Point3D 0.0 0.0 0.0) v4
        drawFace ctx (Face vd1 vd2 vd3 vd4)
    where
